...
//PART2  CREATION ROTATION ARM

var leftArmMaterial = new THREE.ShaderMaterial({
  uniforms:{
    armAngle: armAngle
  }
})

var leftArmShaderFiles = [
  'glsl//arm.vs.glsl',
  'glsl//arm.fs.glsl'
]

new THREE.SourceLoader().load(leftArmShaderFiles,function(shader){
  leftArmMaterial.vertexShader = shaders['glsl//arm.vs.glsl'];
  leftArmMaterial.fragmentShader = shaders['glsl//arm.fs.glsl'];
})
...
var leftLaserMaterial = new THREE.ShaderMaterial({
  uniforms: {
    ...
    LOrR : {
                    type: 'v3',
                    value: new THREE.Vector3(0.25, 0, 0)}
                  }
...
var leftEyeMaterial = new THREE.ShaderMaterial({
  uniforms:{
    ...
    LeftOrRight : {
                    type: 'v3',
                    value: new THREE.Vector3(0.27, 0, 0)}
  }
})
...
lasers.laserGeometry = laserGeometry;
lasers.laserleftMaterial = leftLaserMaterial;
lasers.laserrightMaterial = rightLaserMaterial;


lasers.LLaser = new THREE.Mesh(lasers.laserGeometry, lasers.laserleftMaterial);
lasers.LLaser.position.set(0,0,0);
lasers.LLaser.scale.set(0.6, 0.6, 0.6);

lasers.RLaser = new THREE.Mesh(lasers.laserGeometry, lasers.laserrightMaterial);
lasers.RLaser.position.set(0,0,0);
lasers.RLaser.scale.set(0.6, 0.6, 0.6);

scenes[Part.LASERS].add(lasers.LLaser);
scenes[Part.LASERS].add(lasers.RLaser);
